                    ...
                    <div class="tab-content" id="myTabContent">
                                            ...
                                            <table id="invoice" border="0" cellspacing="0" cellpadding="0">
                                                ...
                                                @foreach($data['pr']->items as $key=>$item)
                                                    <tr id="original" class="original invoice_item" data-id="{{$item->id}}">
                                                        ...
                                                        <td class="cus_price">{{$item->amount ? number_format($item->customer_amount,2,'.','') : '0.00'}}</td>
                                                        <td class="unit editMe">{{$item->amount ? number_format($item->amount,2,'.','') : '0.00'}}</td>
                                                        <td class="qty editMe" data-invoiceQty="{{\App\Services\UtilService::getInvoiceItemTotalQty($item->id)}}">{{$item->qty}}</td>
                                                        ...
                                                        <td class="h_fee">
                                                            <div class="form-group">
                                                                <select id="d_method" class="form-control">
                        ...
                        <div class="tab-pane fade" id="contact" role="tabpanel" aria-labelledby="contact-tab">
                            <table id="example1" class="table table-bordered table-striped">
                                ...
                                <tbody>
                                    @foreach($data['pr']->invoices as $invoice)
                                        <tr>
                                            <td id="invoice_id" data-invoice_id="{{$invoice->id}}">{{$invoice->id}}</td>
        ...
        <div class="modal-dialog modal-xl">
            <div class="modal-content">
                                ...
                                <main>
                                    <div class="row contacts">
                                        ...
                                        <div class="col invoice-details">
                                            ...
                                            <div class="date" id="invoice_date">Invoice Date : {{$data['pr']->created_at}}</div>
                                            {{--<div class="date">Due Date: 30/10/2019</div>--}}
                                        </div>
                                    </div>
                                    <table id="invoice" border="0" cellspacing="0" cellpadding="0">
                ...
                <div class="modal-footer justify-content-between">
                    ...
                    <button type="button" class="btn btn-primary" id="process_purchasing">Process to purchasing</button>
                </div>
            </div>
            <!-- /.modal-content -->
        </div>
...
@section('custom_js')
    ...
    <script>
        ...
        $("#create-invoice").on('click',function () {
            ...
            $.each($(".original .label__checkbox:checked").parents(".invoice_item"), function (i,v) {
                let item = {
                    id : $(v).data('id'),
                    price : parseFloat($(v).find('.unit').text()).toFixed(2),
                    cus_price : parseFloat($(v).find('.cus_price').text()).toFixed(2),
                    qty : parseFloat($(v).find('.qty').text()),
                    tax : parseFloat($(v).find('.tax').text()),
                    h_fee : parseFloat($(v).find('.h_fee').text()),
                    d_method :$(v).find('#d_method').val(),
                ...
                success: function (data) {
                    ...
                    $.each(data, function (i,v) {
                        ...
                        $('#modal-payment').find("#item-container").append('<tr class="invoice_item" data-id="'+v.id+'">' +
                            ...
                            '                                                <td class="no">'+i+1+'</td>' +
                            '                                                <td class="text-left">' +
                            '                                                    <h3>'+v.pr_items.product_name+' - '+v.pr_items.product_code+'</h3>' +
                            '                                                    <p>'+v.pr_items.description+'</p>' +
                            '                                                    <a href="'+v.pr_items.product_url+'" target="_blank" class="hidden-print" >'+v.pr_items.product_url+'</a>' +
                            '                                                    <p>'+v.pr_items.suppliers.name+'</p>' +
                            '                                                </td>' +
                            '                                                <td class="cus_price">'+v.customer_amount.toFixed(2)+'</td>\n' +
        ...
        $(document).on('click',"#btn-view",function () {
            let invoice_no = $(this).parents('tr').find('td#invoice_no').data('invoice_no');
            let invoice_id = $(this).parents('tr').find('td#invoice_id').data('invoice_id');

            $("#process_purchasing").attr("data-invoiceid", invoice_id);
            ...
            $.ajax({
                ...
                success: function (data) {
                    ...
                    $.each(data, function (i,v) {
                        $('#modal-xl').find("#item-container").append('<tr class="invoice_item" data-id="'+v.id+'" data-pr_item_id="'+v.pr_item_id+'">' +
                            '                                                <td>' +
                            '                                                    <div class="center">' +
                            '                                                        <label class="label">' +
                            '                                                        <input  class="label__checkbox" type="checkbox" checked="checked"/>' +
        ...
        $(document).on('click',"#process_purchasing",function () {
            ...
            Swal.fire({
                title: 'Are you sure?',
                text: "You are about to process the invoice to purchasing",
                type: 'warning',
                showCancelButton: true,
                confirmButtonColor: '#3085d6',
                cancelButtonColor: '#d33',
                ...
                if (result.value) {
                    $.ajax({
                        url: "{{route('admin.purchasing.create.do')}}",
                        method: "POST",
                        dataType: "JSON",
                        data: {
                            id: invoice_id,
                        ...
                        success: function (data) {
                            ...
                            if(data.type === 'error'){
                                ...
                                Swal.fire(
                                    'New Purchase added!',
                                    'Invoice has been added for Purchasing.',
                                    'success'
                                );
                ...
                success: function (data) {
                    ...
                    $.each(data, function (i, v) {
                        ...
                        $('#modal-purchasing').find("#item-container").append('<tr class="invoice_item" data-id="' + v.id + '">' +
                            ...
                            '                                                <td class="no">' + (i + 1) + '</td>' +
                            '                                                <td class="text-left">' +
                            '                                                    <h3>' + v.invoice_items.pr_items.product_name + ' - ' + v.invoice_items.pr_items.product_code + '</h3>' +
                                                                                '<div class="form-group">'+
